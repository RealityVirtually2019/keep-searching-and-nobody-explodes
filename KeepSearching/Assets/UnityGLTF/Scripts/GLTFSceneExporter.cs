...
namespace UnityGLTF
{
	public class GLTFSceneExporter
	{
		...
		public byte[] GetGLBBytes(string filename, bool embedImages = true)
		{
			bool cacheEmbedImages = _shouldUseInternalBufferForImages;
			_shouldUseInternalBufferForImages = embedImages;
            ...
            if (jsonWriter == null)
            {
                Debug.Log("Json Writer is null: " + filename);
            }
            _root.Scene = ExportScene(filename, _rootTransforms);
			...
			//			Debug.Log ("Bytesize: " + memStream.Length);

			_shouldUseInternalBufferForImages = cacheEmbedImages;
		...
		private SceneId ExportScene(string name, Transform[] rootObjTransforms)
		{
			...
			foreach (var transform in rootObjTransforms) 
			{
				if (transform.gameObject.activeInHierarchy) {
					scene.Nodes.Add (ExportNode (transform, true));
//					Debug.Log ("Active: " + transform.name);
				} else {
//					Debug.Log ("Not active: " + transform.name);
				}
			}
		...
		private NodeId ExportNode(Transform nodeTransform, bool isRoot)
		{
			var node = new Node();
			...
			{
				if (debugExporter) {
					Debug.Log ("Exporting scene node name: " + nodeTransform.name);
				}
			...
			} else if (isRoot && sceneCamera != null) {
				node.Camera = ExportCamera (sceneCamera);
			}
			...
			if (nonPrimitives.Length > 0)
			{
				...
				foreach (var child in nonPrimitives)
				{
					if (child.activeInHierarchy) {
						bool addThis = false;
						foreach (MeshFilter mf in child.GetComponentsInChildren<MeshFilter>()) {
							if (mf.mesh != null && mf.mesh.vertices.Length > 0) {
								addThis = true;
								break;
							}
						}
						if (addThis) {
							node.Children.Add (ExportNode (child.transform, false));
						}
//						Debug.Log ("Add child node: " + child.name);
					}
				}
			}
...
//				Debug.Log ("Adding prim: " + transform.gameObject.name);
			} else {
//				Debug.Log ("Ignoring prim: " + transform.gameObject.name);
			}
...
//					Debug.Log ("Adding child: " + go.name);
					if (IsPrimitive (go))
						prims.Add (go);
					else
						nonPrims.Add (go);
		...
		{
			if (debugExporter) {
				Debug.Log ("Exporting mesh named: " + name);
			}
			...
			foreach (var prim in primitives)
			{
				MeshPrimitive[] p = ExportPrimitive (prim);
				if (p != null && p.Length > 0) {
					mesh.Primitives.AddRange (p);
				}
			}
		...
		public bool debugExporter = false;
		...
		private MeshPrimitive[] ExportPrimitive(GameObject gameObject)
		{
			...
			if (meshObj.uv.Length != 0 && renderer.material.mainTexture != null)
				aTexcoord0 = ExportAccessor(SchemaExtensions.FlipTexCoordArrayVAndCopy(meshObj.uv));

			if (meshObj.uv2.Length != 0 && renderer.material.mainTexture != null)
		...
		private MaterialId ExportMaterial(Material materialObj)
		{
			...
			if (materialObj.HasProperty("_Cutoff") && material.AlphaMode == AlphaMode.MASK)
			{
			...
			//if (true)//IsPBRMetallicRoughness (materialObj)) 
			//{
            ...
            {
                material.PbrMetallicRoughness = ExportPBRMetallicRoughness(materialObj, IsUnlit(materialObj));
            }
		...
		private PbrMetallicRoughness ExportPBRMetallicRoughness(Material material, bool unlit = false)
		{
            ...
            //    if (_root.ExtensionsUsed == null)
            //    {
            //        _root.ExtensionsUsed = new List<string>(new[] { "KHR_materials_unlit" });
            //    }
            //    else if (!_root.ExtensionsUsed.Contains("KHR_materials_unlit"))
            //    {
            //        _root.ExtensionsUsed.Add("KHR_materials_unlit");
            //    }
            ...
            //        if (_root.ExtensionsRequired == null)
            //        {
            //            _root.ExtensionsRequired = new List<string>(new string[] { "KHR_materials_unlit" });
            //        }
            //        else if (!_root.ExtensionsRequired.Contains("KHR_materials_unlit"))
            //        {
            //            _root.ExtensionsRequired.Add("KHR_materials_unlit");
            //        }
            //    }
            //    //pbr.Extensions = new Dictionary<string, IExtension>() { { "KHR_materials_unlit", null } };
            //}
		...
		{
			//Debug.Log ("Exporting texture " + textureObj.name + " with type " + textureMapType);
		...
		{
			//Debug.Log ("Exporting image " + texture.name + " with type " + textureMapType);
			ImageId id = GetImageId(_root, texture);
			if (id != null)
			{
                //Debug.Log("FOUND IMAGE ID: " + id);
				return id;
			}
		...
		public MaterialId GetMaterialId(GLTFRoot root, Material materialObj)
		{
			...
			{
				if (_materials[i] == materialObj || _materials[i].name.Equals(materialObj.name))
				{
                    if (_materials[i].color.Equals(materialObj.color))
                    {
		...
		public TextureId GetTextureId(GLTFRoot root, Texture textureObj)
		{
			for (var i = 0; i < _textures.Count; i++)
			{
				if ((_textures[i] == textureObj) ||
                    (_textures[i].name.Equals(textureObj.name) &&
                     _textures[i].GetInstanceID() == textureObj.GetInstanceID()))
				{
		...
		{
            //Debug.Log("Find existing image id for texture: " + imageObj.name);
			for (var i = 0; i < _imageInfos.Count; i++)
			{
				if (_imageInfos[i].texture == imageObj)
				{
                    Debug.Log(i + ": Found image id for texture: " + imageObj.name);
					return new ImageId
					{
						Id = i,
						Root = root
